...
def build_probe_model(initializers: dict[str, np.ndarray], rows: int) -> onnx.ModelProto:
    ...
    model = helper.make_model(graph, producer_name="magcode_qairt_mlp_probe")
    model.ir_version = 8
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export a QAIRT-friendly real-weight MLP probe without LayerNorm.")
    parser.add_argument("--model", required=True, help="Path to the MLP+LayerNorm ONNX model")
