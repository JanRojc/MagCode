...
read_json_field() {
  local field="$1"
  python3 - "$CASE_DIR/case_manifest.json" "$field" <<'PY2'
import json, sys
...
"$QNN_ONNX_CONVERTER"   --input_network "$CASE_DIR/block_0_1_edge_mesh_mlp_body.onnx"   --input_dim "$INPUT_NAME" "${ROWS},${INPUT_DIM}"   --output_path "$BUILD_DIR/${MODEL_NAME}.cpp"

echo "Building Android model library"
"$QNN_MODEL_LIB_GENERATOR"   -c "$BUILD_DIR/${MODEL_NAME}.cpp"   -b "$BUILD_DIR/${MODEL_NAME}.bin"   -l "$MODEL_NAME"   -o "$BUILD_DIR/model_libs"   -t aarch64-android
