...
if [[ ! -x "$QNN_ONNX_CONVERTER" ]]; then
  echo "ERROR: missing converter: $QNN_ONNX_CONVERTER" >&2
  exit 1
fi
if [[ ! -x "$QNN_MODEL_LIB_GENERATOR" ]]; then
  echo "ERROR: missing model lib generator: $QNN_MODEL_LIB_GENERATOR" >&2
  exit 1
fi
...
"$QNN_ONNX_CONVERTER" \
  --input_network "$CASE_DIR/mlp_body.onnx" \
...
echo "Next: optionally run ./run_probe_android.sh, then copy the whole case dir back to the Mac."
